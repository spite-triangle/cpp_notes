...
date: "2024-03-03"
draft: false
description: "Docker数据卷使用指南，涵盖挂载方式、匿名/具名挂载及容器间共享存储。"
weight: 8
tags: ["Docker Volume", "数据持久化"]
categories: ["核心原理"]
---
...
```term
$ docker run -it -v 本机目录:容器目录 image
$ docker run -dt -v /home/triangle/centos:/home centos
$ lsblk
...
```term
$ docker run -dt -v /home centos # # 匿名挂载
$ docker run -dt -v name:/home centos # 具名挂载
...
```term
$ docker volume ls -q |xargs docker volume rm # 删除所有数据卷文件
$ docker volume ls  
DRIVER    VOLUME NAME
$ docker run -dt -v /home/triangle/workspace/test/:/home centos # 指定路径挂载
32a0de93ca92349b8d246d6ff15e23d872dbc23cb9fa0ce5f1a553526d21f879
$ docker volume ls  
DRIVER    VOLUME NAME
$ docker inspect 32a0de93ca92
    ...
        "Mounts": [
            {
                "Type": "bind",                             # 模式为绑定
...
local     d0677d8fbc756eba67ccc395a0ed42fc61433582905f161296f8e2519774989e
local     name                                                              # 具名挂载
$ docker volume rm name # 删除数据卷在本机上的数据文件夹
...
```term
$ cd /var/lib/docker/volumes/  # linux 版卷路径
$ dir \\wsl.localhost\docker-desktop-data\data\docker\volumes # wsl2 版卷路径
...
```term
$ docker build -f dockerfile -t triangle/centos:1.0 ./ # 生成镜像
...
![shared](./resources/volume_shared.jpg)
...
```term
$ docker run -dt --name centos01 triangle/centos:1.0 # 启动上面自定义的镜像
d69c26e04acb2cfcf38dcba1619a036e5435ba8469c8316e2f76e1075772eb52
$ docker run -dt --name centos02 --volumes-from centos01  triangle/centos:1.0 
