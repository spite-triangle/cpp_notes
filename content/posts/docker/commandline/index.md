...
date: "2024-03-02"
draft: false
description: "Docker命令行操作大全，涵盖镜像管理与容器启停等常用指令。"
weight: 2
tags: ["Docker CLI", "命令行工具"]
categories: ["工具使用"]
---
...
![command](./resources/command.jpg)

# 帮助命令

```term
$ docker version # 版本信息
$ docker version # 系统信息，包括镜像和容器数量
$ docker --help  # 帮助
```
...
$ docker images # docker image ls 的简写
...
```term
$ docker search [name] 
$ docker search --filter=STARS=200 mysql # --filter 筛选条件
...
```term
$ docker pull [name:tag] # docker image pull 的别名
$ docker pull mysql
...
$ docker pull docker.io/library/mysql:latest # 与上面等价
```
...
hello-world   latest    d2c94e258dcb   10 months ago   13.3kB
$ docker rmi a88c3e85e887 # 根据 IMAGE ID 删除，rmi 中的 i 是 image
...
$ docker run -dt centos  # 给 centos 创建了一个终端
...
6ff2861f85cd   hello-world   "/hello"      2 hours ago          Exited (0) 2 hours ago                thirsty_lamport
$ docker exec -it c66d975d5cd8 /bin/bash # 通过容器执行一个命令，然后进入
$ 
```
...
$ docker attach a8ac22219a85 # 将容器后台运行内容展示到前台
hello
hello
hello
hello
...
6ff2861f85cd   hello-world   "/hello"      2 hours ago          Exited (0) 2 hours ago                thirsty_lamport
$ docker rm c66d975d5cd8 
c66d975d5cd8    # 删除成功
$ docker rm cb3cce70496c # 不能直接删除正在运行的，可以用 -f 强制删除
...
$ docker logs -n 10 -t a8ac22219a85 | cat -n
...
$ docker cp c2de15be257e:/home/test.txt ./ # 从容器的虚拟磁盘中将文件拷贝出来
