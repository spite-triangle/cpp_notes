...
date: "2024-05-12"
draft: false
description: "Shell命令解析器工作原理，涵盖命令行语法树构建、进程执行模型及终端与Session管理机制"
weight: 5
tags: ["shell", "命令行解析", "语法树", "管道", "重定向", "C标准库", "malloc"]
categories: ["操作系统基础", "用户接口"]
---


# shell

![shell](./resources/shell.jpg)

- **kernel** : 提供操作系统的系统调用
- **shell** : 与人交互的程序，用户客户通过 shell 来使用操作系统的 kernel

```term
$ main sh # 最简单的 linux shell 文档
```
...
```term
$ gcc -c -ffreestanding sh-xv6.c -g -o2 # -ffreestanding 零依赖，包含系统库
$ ld sh-xv6.o -o sh
$ ./sh
> /bin/pwd
/home/triangle
...
```term
$ (/bin/echo hello > test.txt ; /bin/cat test.txt) | /bin/wc -l
```

上述指令通过语法树可以解析为：

![expression tree](./resources/expressionTree.jpg)
...
$ tty # 查看终端名
/dev/pts/0
$ echo > /dev/pts/1 # 将打印信息重定向到 1 终端
$ tmux # 创建多个终端，实现界面分屏
```

# Session

![Session ](./resources/session.png)
