...
date: "2024-03-02"
draft: false
description: "进程生命周期管理核心机制，涵盖fork创建、execve加载、exit退出及虚拟地址空间与mmap内存映射"
weight: 4
tags: ["进程管理", "fork", "execve", "mmap", "vdso", "写时复制"]
categories: ["操作系统基础", "系统编程"]
---
...
![alt](./resources/processRelationship.png)

操作系统内核加载后，会去启动一个进程，之后所有进程都是通过该进程或者子进程进行创建。一般启动的就是 `systemd` 进程。

![alt](./resources/initProcess.png)
...
- **建立平行世界**：根据操作系统是状态机的特点，建立某个状态的快照，实现状态回退以及读档
    ...
    ![parallel](./resources/parallel_fork.png)
...
```term
$ man vdso # 可以查看其它不用进入操作系统内核，就能获取结果的系统调用
```
## mmap
