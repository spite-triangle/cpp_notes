...
date: "2024-03-02"
draft: false
description: "文件系统核心原理与实现，涵盖FAT/ext2目录树结构、文件挂载机制及RAID磁盘阵列技术"
weight: 9
tags: ["文件系统", "FAT", "ext2", "inode", "mount", "RAID", "日志", "崩溃一致性"]
categories: ["操作系统基础", "存储系统"]
---
...
```term
$ sudo mount game.img /mnt # 将 game.img 挂载到 /mnt 文件目录
```
...
```term
$ ln a.txt b.txt # 硬链接，不能链接目录，不能跨文件系统
...
![alt](./resources/fileSystemStructure.jpg)
...
![block link](./resources/blockLink.jpg)

**方案二**：将块划分为两种类型，一种用于存储文件相关的块指针，一种用于存储数据。
- 数据块指针存放集中，数据指针块完蛋，文件就完蛋 

![block classification](./resources/blockClassification.jpg)
...
![fat Entry](./resources/fatEntry.jpg)

**「目录」在文件系统则是一个特殊的文件**。在 FAT 中，目录文件存放的是一组 Directory Entry (描述了文件名，文件权限等)

![directory entry](./resources/directoryEntry.jpg)
...
```term
$ yes | head -c 104857600 > fs.img # 先创建 100m 文件
$ mkfs.fat -f 4 -s 8 -S 512 -v fs.img # 格式化文件为 FAT 文件系统
...
$ sudo mount fs.img /mnt/img # 将虚拟文件系统挂载到操作系统
$ sudo touch /mnt/img/test.txt # 往文件系统添加内容
```
...
![ext2 folder](./resources/ext2Folder.jpg)
