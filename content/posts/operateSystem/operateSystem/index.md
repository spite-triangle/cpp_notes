...
date: "2024-03-02"
draft: false
description: "操作系统从CPU初始化到固件加载再到内核启动的完整引导流程分析"
weight: 3
tags: ["系统启动", "BIOS", "UEFI", "中断", "Firmware", "CPU初始化"]
categories: ["操作系统基础"]
---
...
![调试](./resources/stateMachineDebug.png)
...
![性能](./resources/perfCheck.png)

操作系统利用中断，每个一段时间去统计一下程序状态机的运行状况，就能将程序性能消耗情况统计出来。根据统计结果，就能判断那部分是需要优化的

```term
$ pref stat ./a.out
$ pref record ./a.out # 记录统计结果
$ pref report ./a.out # 查看统计结果
```
