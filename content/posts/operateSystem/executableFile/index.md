...
date: "2024-05-12"
draft: false
description: "ELF可执行文件格式深度解析，涵盖静态链接重定位、动态加载器原理及PLT/GOT符号绑定机制"
weight: 6
tags: ["ELF格式", "动态链接", "静态链接", "GOT", "PLT", "重定位", "objdump"]
categories: ["操作系统基础", "编译链接"]
---
...
```term
$ addr2line 0x1189 a.out # 将可执行文件中的二进制地址翻译成源文件行号
...
```term
$ g++ unwind.c  -g -O0 -static -fno-omit-frame-pointer # static: 静态链接；fno-omit-frame-pointer 生成栈帧
$ ./a.out  | xargs addr2line a.out 
...
```term
$ gcc dlbox.c -o dlbox # 编译自定义工具链
$ ./dlbox gcc libhello.S # 编译动态库
$ ./dlbox gcc libc.S # 编译动态库
$ ./dlbox gcc main.S # 主程序
$ ./dlbox interp main.dl  # 执行主程序
Hello 1
```

### 函数跳转

![call](./resources/call.jpg)
...
├── add2.c
└── main.c
$ gcc add1.c -I. -shared -fPIC  -o libadd1.so
$ gcc add2.c -I. -shared -fPIC  -o libadd2.so
$ gcc main.c -L. -ladd1 -ladd2 -Wl,-rpath=./
$ ./a.out
add1
$ gcc main.c -L. -ladd2 -ladd1 -Wl,-rpath=./
$ ./a.out
add2
```
...
```term
$ ar -t  ./bin/add_static/libadd_static.a
add1.cpp.o
add.cpp.o
$ lib.exe /list .\bin\add_static\add_static.lib
...
源码与静态库的「标准库」形式不统一（使用静态库，还是动态库），存在链接冲突，即 `MT` 与 `MD` 冲突
