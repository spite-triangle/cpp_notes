...
date: "2024-03-02"
draft: false
description: "Xv6教学操作系统的核心实现机制，涵盖虚拟内存映射、进程调度策略及任务切换原理"
weight: 7
tags: ["Xv6", "虚拟内存", "任务调度", "CFS", "优先级继承", "RISC-V", "trampoline"]
categories: ["操作系统实践", "Xv6源码分析"]
---
...
![alt](./resources/xv6-syscalls.png)
...
![memory](./resources/xv6-process-memory.png)
...
```term
$ taskset -c 0 yes # 将进程绑定到指定 cpu 执行
$ nice -n 19  yes # 指定进程的 niceness 值
$ taskset -c 0 nice -n 9 yes > /dev/null &
$ taskset -c 0 nice -n 19 yes > /dev/null &
$ top # NI 就是 niceness 值；两个 yes 进程 cpu 占比 9 ：1
...
```term
$ gcc -MD -c main.c # MD 告诉编译器生成 .d 文件
$ ll
...
```term
$ bear make target # bear 会解惑编译指令，生成 compile_commands.json
```
