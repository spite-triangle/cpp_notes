...
date: "2024-03-02"
draft: false
description: "I/O设备模型与驱动设计，涵盖总线寻址、中断控制器、DMA机制及GPU的SIMD/SIMT架构原理"
weight: 8
tags: ["I/O设备", "DMA", "中断控制器", "GPU", "SIMT", "CUDA", "块设备驱动"]
categories: ["操作系统基础", "硬件接口"]
---
...
![io Devices](./resources/iodevice.jpg)
...
$ sudo apt-cache search linux-source # 查看 linux 内核
$ sudo apt-cache search linux-source
linux-source - Linux kernel source with Ubuntu patches
linux-source-5.4.0 - Linux kernel source for version 5.4.0 with Ubuntu patches
。。。
$ sudo apt-get install linux-source-5.4.0 # 下载源码，位置 /usr/src
```
...
```term
$ insmod  module.ko # 安装驱动 .ko 文件，只完成了注册
$ lsmod # 查看安装的驱动
$ mknod /dev/[name] c 237 0// 创建设备文件，name: device_create 函数给定的
```

5. 使用设备

```term
$ cat /dev/[name] # 对设备进行读操作
$ echo hello > /dev/[name] # 对设备进行写
$ dmesg # 查看内核 msg 信息 printk 函数打印的
```
