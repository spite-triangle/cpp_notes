...
date: "2025-04-27"
draft: false
description: "Electron框架基本概念，对比传统方式与Vue3脚手架方案含调试打包指南。"
weight: 1
tags: ["Electron", "基础入门", "跨平台桌面开发"]
categories: ["核心特性"]
---
...
![alt](./resources/electron_structure.png)
...
```term
$ npn init -y   # 初始化 nodejs 工程
$ npm install -g cnpm --registry=https://registry.npmmirror.com # 安装国内版 npm
$ cnpm install --save-dev electron # 安装 electron
$ tree .
.
├── node_modules        # 模板文件夹
├── index.html          # 主界面
...
```term
$ npm install -g cnpm --registry=https://registry.npmmirror.com # 安装国内版 npm
$ npm create @quick-start/electron # 通过脚手架创建项目
$ cnpm i # 安装依赖包，使用 npm 容易卡住
```
...
```term
$ # 在 powshell 中直接输入以下内容
if (!(Test-Path -Path $PROFILE)) {
    New-Item -ItemType File -Path $PROFILE -Force
}
$ notepad $PROFILE # 打开上面创建的脚本文件，修改代理地址
...
```term
$ npm run build:win # 打包 window 程序
$ npm run build:upack # 只生成可执行程序，不打包 
```
