...
date: "2024-05-12"
draft: false
description: "MSVC是Microsoft Visual C++编译器，提供完整的命令行编译、链接及调试配置。"
weight: 1
tags: ["MSVC", "cl.exe", "link.exe", "PDB", "动态库静态库"]
categories: ["编译器", "Windows开发"]
---
...
```term
$ cl /EHsc /nologo /O1 /GF /Feout /std:c++11 main.cpp add.cpp
```
- **/EHsc** : 指定异常处理模型
...
├── build
└── main.cpp
$ cl /I"./add/" /Fo"./build/" /Fe"./bin/"  main.cpp "./add/add.cpp" 
$ tree
.
├── add
│   ├── add.cpp
...
```term
$ cl /c add.cpp # 生成 .obj 二进制文件
$ lib add.obj # 利用 lib.exe 生成 .lib 静态库文件
$ cl main.cpp add.lib # 编译且链接成目标文件 link 的步骤一起做了
```
...
```term
$ cl /LD /D_WINDLL add.cpp # 编译动态库
$ tree 
.
├── add.obj
├── add.lib
└── add.dll
$ cl main.cpp add.lib # 生成目标文件
```
...
```term
$ dumpbin /EXPORTS add.lib # 查看静态库的
...
File Type: LIBRARY
  ...
  Summary
          ...
          45 .drectve
          15 .text$mn
$ dumpbin /EXPORTS add.lib # 查看动态库的
Dump of file add.lib
...
**查看lib类型：**
```term
$ lib /list add.lib # 查看静态库
add.obj
$ lib /list add.lib # 查看动态库
add.dll
```
...
```term
$ cl /source-charset:<IANA_name|.CPID> /execution-charset:<IANA_name|.CPID>
$ chcp <.CPID> # 切换 powershell 的显示字符集
```
...
├── build
└── main.cpp
$ cl.exe /c /Fo"./build/" /Fd"./build/" /Zi .\main.cpp .\add\add.cpp 
$ tree
.
├── add
│   ├── add.cpp
...
│   └── vc140.pdb
└── main.cpp
$ link.exe .\build\main.obj .\build\add.obj /OUT:"./bin/test.exe" /DEBUG 
$ tree
.
├── add
│   ├── add.cpp
...
├── lib
└── main.cpp
$ cl /c /Zi /Fo"./build/" /Fd"./build/add.pdb" .\add\add.cpp # 一定要将 obj 与 pdb 放到同一文件夹
$ tree
 .
├── add
│   ├── add.cpp 
...
├── lib
└── main.cpp
$ lib .\build\add.obj /OUT:"./lib/add.lib" 
$ tree
 .
├── add
│   ├── add.cpp
...
├── lib
└── main.cpp
$ cl /c /LD /D_WINDLL /Zi /Fo"./build/" /Fd"./build/add.pdb" .\add\add.cpp # 一定要将 obj 与 pdb 放到同一文件夹
$ tree
 .
├── add
│   ├── add.cpp 
...
├── lib
└── main.cpp
$ link /DLL /OUT:"./lib/add.dll" /DEBUG .\build\add.obj
$ tree
 .
├── add
│   ├── add.cpp 
...
```term
$ # 以 debug 模式，编译 solutionName.sln 中的 projectName 项目 
$ devenv solutionName.sln /Project projectName /Build Debug
```
