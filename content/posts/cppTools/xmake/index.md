...
date: "2024-12-28"
draft: false
description: "XMake是轻量级跨平台构建工具，使用Lua配置描述项目目标、依赖和编译规则。"
weight: 1
tags: ["XMake", "Lua", "xmake.lua", "多平台", "target配置"]
...
```term
$ xmake create -l [language] -p [path]
```
...
```term
$ xmake globle --mingw=[mingw路径] # 配置编译器路径
$ xmake config -p [mingw|windows] --sdk=[mingw路径] # -p 代表 plat 
```
...
```term
$ xmake config -m debug # 切换到debug，需要在 build 之前运行
$ xmake build
```
