...
date: "2026-03-08"
draft: false
description: "行为树的设计原理、节点类型及与状态机的对比分析，含Python实现示例，适用于AI和业务流程编排。"
weight: 8
tags: ["行为树", "AI决策", "游戏开发"]
categories: ["设计模式"]
---
...
![alt](./resources/bt_robot.png)

1. 走到位置 `A`，然后找到物体

    ![alt](./resources/bt_toa.png)

2. 在抵达 `A` 的过程中，机器人肯定要实时检测是否到达了 `A`，因此，还需加入是否抵达位置的判断

    ![alt](./resources/bt_check_toa.png)

3. 添加其他待检测地点

    ![alt](./resources/bt_locations.png)

4. 增加被搜索物品

    ![alt](./resources/bt_objects.png)
...
![alt](./resources/robot_grip.png)

现在追加一个流程，机器人夹取物品时，需要检测夹取是否有效，且必要时进行修正

![alt](./resources/robot_correct.png)

可以看出行为树流程更加清晰，符合直觉，且便于拓展。

**案例2**：机器人增加充电功能

![alt](./resources/robot_charge_bt.png)
![alt](./resources/robot_charge_state.png)
...
![alt](./resources/robot_charge_mix.png)
