...
date: "2026-06-27"
draft: false
description: "Node.js 运行时环境，提供本地执行生态与 npm 包管理系统"
weight: 6
tags: ["nodejs", "npm", "包管理", "SDK版本", "fnm"]
categories: ["前端基础"]
cover: resources/nodejs.jpg
---
...
```term
$ npm config list -l # 配置
$ npm config set registry https://registry.npmmirror.com # 修改源
$ npm config set prefix [dir] # 设为模块的全局安装目录
$ npm config set init.author.name [name] # 配置 package.json 初始化设置
```
...
$ npm search [name] # 在仓库搜索
$ npm list # 查看当前目录下的包
$ npm update [package name] # 升包
```

- **运行**

```term
$ npm run [scripts] # 执行 package.json 中配置的脚本
...
```term
$ npm link  # 将当前「正在开发的包」临时放入全局包
$ npm link [package] # 其他项目引用放入全局包的「临时包」
$ npm unlink [package] # 删除临时包
```

- **发布**

```term
$ npm publish [--tag beta] # 发布到 npmjs.com
```
...
$ npm run [scripts] # 运行 package.json 中脚本
```
