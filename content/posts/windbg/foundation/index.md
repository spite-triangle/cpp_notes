...
date: "2024-03-02"
draft: false
description: "介绍 Windbg 调试器的工作原理，包括软硬件断点、调试事件分发和 SEH 异常处理机制。"
weight: 1
tags: ["Windbg", "断点", "SEH", "异常分发", "PDB符号"]
categories: ["开发工具"]
---
...
```term
$ u ntdll!NtReadFile # 查看 ntdll 中的 NtReadFile 函数
ntdll!NtReadFile:
...
$ u 778c2bf0 # 用另外一个 windbg 以 non-invasive attach 形式调试
ntdll!NtReadFile:
...
![non-invasive attach](./resources/nonInvasive.jpg)

## 硬件断点

![debug register](./resources/debugRegister.jpg)
...
![JTAG](./resources/jtag.jpg)

# 用户态调试

## windows 调试

![user debug model](./resources/userDebugModel.png)
...
```term
$ x nt!Dbgk* # x 展示符号。nt!Dbgk* 为内核中用于支持用户态调试的函数
$ x nt!kd* # nt!kd* 内核相关的调试函数
$ dt _EPROCESS # dt 查看数据结构内容，_EPROCESS 操作系统管理进程的主要结构体
ntdll!_EPROCESS
   +0x000 Pcb              : _KPROCESS
...
![debug object](./resources/debugObject.jpg)
...
clrn - CLR notification exception - second-chance break - handled
 ...
 ...
$ sxe ld # 修改 ld 事件状态，事件发生就会在 first chance handling 中断程序
$ sx 
  ...
...
03 0462fb90 778b7c3e     ntdll!__RtlUserThreadStart+0x2f
04 0462fba0 00000000     ntdll!_RtlUserThreadStart+0x1b
$ u # ntdll!DbgBreakPoint 函数二进制
ntdll!DbgBreakPoint+0x8:
778c4f28 cc              int     3
778c4f29 cc              int     3
778c4f2a cc              int     3
$ bp ntdll!NtReadFile ".echo fuck;k;" # 为断点设置命中后执行的脚本
$ .restart /f # 重启进程
```
...
![altKiDispatchException](./resources/KiDispatchException.jpg)
...
$ bu demo!filter # 在 filter 函数设置断点，u 是符号，p 是地址
$ g # 继续运行
Breakpoint 0 hit
demo!filter:
...
$ dx -r1 ((demo!_EXCEPTION_POINTERS *)0xa07aafe920) # 查看结构体
...
![create pdb](./resources/createPDB.jpg)

> [!note]
> `pdb` 即 `program database` ，其本质就是存储调试信息的数据库

![pdb ](./resources/pdbFile.jpg)

```term
$ dumpbin.exe /PDBPATH:VERBOSE .\bin\demo.exe # 查看 demo.exe 的 pdb 文件。在 demo.exe 中会存储 pdb 文件信息
...
File Type: EXECUTABLE IMAGE
 找到 PDB 文件(在“E:\testspace\cpp\bin\demo.pdb”中)
    ...
$ llvm-pdbutil.exe pretty  -include-compilands='main.cpp*'    -module-syms  -include-symbols='filter|main'   .\bin\demo.pdb
...
$ llvm-pdbutil.exe pretty  -include-compilands='main.cpp*'  -lines  .\bin\demo.pdb  # -lines 查询行号与符号对应关系
...
$ g++ test.cpp -g -O0
$ readelf --debug-dump=decodedline  a.out # 查看存储的源码行号
