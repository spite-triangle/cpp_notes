...
date: "2024-03-02"
draft: false
description: "Windows 线程调试技术，涵盖 EPROCESS/TEB 数据结构、临界区分析和多线程调试命令。"
weight: 7
tags: ["CRITICAL_SECTION", "临界区", "multithreading", "_KETHREAD", "Windbg调试"]
categories: ["系统编程"]
---
...
```term
$ dt nt!_EPROCESS  # 内核查看进程
$ dt nt!_KETHREAD # 内核态线程
$ dt nt!_PEB # 用户态查看进程
$ dt nt!_TEB # 用户态查看线程
$ dt _CONTEXT # 线程切换的上下文
``` 

# 调试命令

```term
$ .create cmd.exe # 打开进程，同时调多个进程
CommandLine: cmd.exe
# 运行一次 g 才会启动进程
...
$ ~1n # 增加 1 号的线程的挂起计数
$ ~1m # 减少 1 号的线程的挂起计数
$ ~1f # 冻结线程。当detach进程后，冻结会解除，而挂起计数则不会减少，即冻结会自动恢复，挂起计数则不会
$ ~* k # 所有线程均执行 k 命令
```
...
$ dt _RTL_CRITICAL_SECTION_DEBUG
$ !cs # 查看所有临界区
$ !locks # 查看所有锁住的临界区
```
