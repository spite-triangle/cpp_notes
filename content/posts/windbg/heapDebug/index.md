...
date: "2024-04-13"
draft: false
description: "Windows 堆管理机制与内存泄漏检测，涵盖页堆、UMDH、PerfView GC Heap 分析等工具链。"
weight: 6
tags: ["堆管理", "页堆", "UMDH", "CRT堆", "内存泄漏", "GC Heap"]
categories: ["性能优化"]
---
...
![heap](./resources/memoryManager.jpg)
...
![heap manager](./resources/heapManager.jpg)
...
![heap structure](./resources/heap_structure.jpg)
...
$ !heap
        Heap Address      NT/Segment Heap
             ...
             1070000              NT Heap
$ dt _HEAP 1070000 -r2 # 将 1070000 地址的数据转化为 _HEAP 结构体进行展示
...
$ !heap -x 01070000 # 第一个堆块
Entry     User      Heap      Segment       Size  PrevSize  Unused    Flags
-----------------------------------------------------------------------------
01070000  01070008  01070000  01070000       4a8         0         1  busy 
$ !heap -x 01070000+4a8 # 第二个堆块，可以通过 Size 与 PrevSize 进行段块的前后索引
Entry     User      Heap      Segment       Size  PrevSize  Unused    Flags
-----------------------------------------------------------------------------
...
```term
$ # 编译器使用的是 msvc 2017，因此 crt 堆就是用的默认堆
$ !heap
        Heap Address      NT/Segment Heap
...
$ !heap 7c0000 -a # delete [] 后再次查看
..
# new 的堆块已经变成的 free 块
00df3dc0: 00040 . 00038 [104] free fill
..
$ dd 00df3dc0 # delete [] 后再次查看
...
$ dd 015c3dc0 # fill(buffer, len + 2 , 'x') 执行后
...
... 
$ g # 不管 WARNING 继续运行 buffer = nullptr;
$ dd 015c3dc0 # 查看块释放情况，可以看到内存根本没有释放
015c3dc0  7475e0ea 1c00f683 78787878 78787878
015c3dd0  78787878 abab7878 abababab feeefeee
015c3de0  00000000 00000000 7076e0ed 0000f68e
015c3df0  015c4028 015c00c0 7475e0ea 1800f689
$ !heap 15c0000 -v # 手动执行堆检查
    ...
...
```term
$ .\gflags.exe -i demo.exe +hpa # 指定程序启用 DPH
$ .\gflags.exe -i demo.exe -hpa # 撤销 DPH
$ .restart # gflags 的设置重启进程后生效
$ !gflag # 查看启用状态
...
![normal page heap](./resources/normalPageHeap.jpg)
...
```term
$ !eeheap -gc # 查看 .net 应用的 GC 内存分配情况
...
$ !heap -p -a 0x05E44FF0   # 泄漏点定位 ，需要配合 gflags.exe -i demo.exe +hpa 才能使用
```
...
![memory code](./resources/memoryCode.jpg)
...
```term
$ cmd # 切换到 cmd 才能使用 set 命令
$ set _NT_SYMBOL_PATH=pdb_path # 设置 pdb 文件路径，可选
$ set OANOCACHE=1 # 禁用 BSTR 缓存，这个会导致追踪结果不准确
$ powershell.exe 
$ .\gflags.exe -i demo.exe +ust # 创建运行时堆栈跟踪数据库
$ .\demo.exe # 启动程序
$ umdh.exe -pn:demo.exe -f:mem1.log # 抓取快照
$ umdh.exe -pn:demo.exe -f:mem2.log # 程序运行一段时间，再抓取快照
$ umdh.exe -d mem1.log mem2.log > res.log # 比对两个日志，得到分析结果
```
...
![vmmap field](./resources/vmmap_fields.png)

# PerfView

## GC Heap 

![gc heap](./resources/perfview_GC_Heap.png)
...
![Unmanaged Memory](./resources/perfview_unmanaged_memory.png)
...
![os heap](./resources/perfview_os_heap.png)



## Diff

![diff](./resources/perfview_diff.png)
