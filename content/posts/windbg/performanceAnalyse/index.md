...
date: "2024-03-02"
draft: false
description: "Windows 性能分析工具链，涵盖 PerfView、WPA 和 ETW 的事件追踪与 CPU/线程阻塞分析方法。"
weight: 4
tags: ["PerfView", "WPA", "ETW", "xperf", "CPU Stack分析"]
categories: ["性能优化"]
---
...
![perf view](./resources/perfview_run.jpg)

- `Collect` : 日志捕获的开始和结束，由使用者自己决定

![perf collect](./resources/perfview_collect.jpg)

```term
$ prefview /? # 帮助手册
$ prefview collect  /nogui # 开始收集，并不展示界面。默认收集缓存最大为 500 m
```
...
![prefview open](./resources/perfview_open.jpg)
...
![cpu stack](../../image/windbg/cpu%20stack.jpg)
<!-- WARNING: resource not found: ../../image/windbg/cpu%20stack.jpg -->
...
![call tree](./resources/perfview_calltree.jpg)

通过调用树，根据 `Inc` 值就能找到 CPU 占用最高的函数和线程，即**自上而下分析法** (`Top-Down analyses`)

### 自下而上

![by Name](./resources/perfview_byname.jpg)
...
![caller](./resources/perfview_caller.jpg)

选中要查看的函数，然后按照上图操作，就能展示出该函数在哪儿被调用了

![callers](./resources/perfview_callers.jpg)

### 分组

![group](./resources/perfview_group.jpg)

- `Fold%` : 函数的 `Inc %` 小于指定值，展示上就会被隐藏，用于杂音过滤
- `GroupPats` : 类似于筛选器的东西，会对所有的符号和函数进行筛选。

![dril into](./resources/perfview_drillinto.jpg)
...
![Event Filter](./resources/perfview_eventFilter.jpg)
...
![histogram](./resources/perfview_histogram.jpg)

x 轴为事件采样时间；y 轴为当前时间有多少事件的数字。选中数字后，在左下角会展示当前数字对应的时间

## 阻塞时间分析

![block time](./resources/blockTime.jpg)
...
![thread time](./resources/perfview_threadTime.jpg)
![thread time](./resources/threadTime.jpg)
...
![cpu time](./resources/cpuTime.jpg)
...
![wpa](./resources/wpa.jpg)

3. 配置 `pdb`。**只配置目标进程的，不然会下载全部的符号文件。**

![wpa config](./resources/wpa_config.jpg)

4. 点击 `Load Symbols` 就能查看结果了

![wpa result ](./resources/wpa_result.jpg)
...
![analyses](./resources/visualStudio_analyse.jpg)

2. 指定进程

![launch](./resources/visualStudio_launch.jpg)

3. 生成报告

![report](./resources/visualStudio_report.jpg)

4. 分析报告，分析方法见 `Perfview`

![analyse report](./resources/visualStuido_analyseReport.jpg)
