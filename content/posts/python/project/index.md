...
date: "2025-09-24"
draft: false
description: "Python项目管理笔记，涵盖conda环境管理与poetry等包管理工具对比。"
weight: 5
tags: ["依赖管理", "conda", "Poetry"]
categories: ["工程实践"]
---
...
$ conda config --add channels https-url # 添加源
$ conda config --remove channels https-url # 删除源
$ conda config --set show_channel_urls yes # 设置安装包时，显示镜像来源
```
...
```term
$ cd miniforge3\condabin # 安装目录下的 condabin 文件夹
$ ./conda.bat init # 初始化环境
$ 
$ 
...
```term
$ cd miniforge3/bin # 安装目录的 bin 文件夹
$ ./conda init # 初始化
$ source ~/.brashrc
```
...
$ conda install mamba -n base -c conda-forge # 安装 mamba
```
...
```term
$ python.exe -m venv .env # 创建虚拟环境，第三方包放在 .env 文件夹下
$ .\.env\Scripts\activate # windows 激活环境
$ source ./.env/bin/activate # linux 激活环境
...
```term
$ pip install -e . # 以 dev 模式安装项目 
$ hello # 执行 project.scripts 命令
```
...
Commands
    ...
    publish                 发布包到公网 
$ poetry export -f requirements.txt -o requirements.txt # 根据 pyproject.toml 生成 requirements.txt
```
...
$ poetry config virtualenvs.in-project true # 虚拟环境创建在项目目录，方便管理
$ poetry env use python # 根据当前 python 版本创建虚拟环境，也可以指定 python.exe 的绝对路径
$ poetry env activate # 只是打印进入虚拟环境的命令，具体指令见 venv 章节
$ poetry run [command] # 在虚拟环境中执行 command 命令
```
...
$ poetry.exe source add aliyun  xxxxx # 添加源
$ poetry.exe source remove aliyun # 删除源
$ poetry.exe config --list
    ...
...
```term
$ poetry add [package] # 同 pip install [package] ，运行环境依赖
$ poetry add [package] --group dev # 开发环境依赖
$ poetry add [package] --group test # 测试环境依赖
$ poetry remove [package] # 删除包
$ poetry install --help
Options
...
$ poetry update --lock
$ poetry lock
$ poetry check  # 检查 poetry.lock 与 pyproject.toml
```
...
```term
$ pip install poetry-plugin-* # 安装插件
```

若想实现 `npm run [command]` 的效果，需要借助脚本实现

```term
$ pip install poetry-plugin-taskipy # 安装方案一
$ poetry self add poetry-plugin-taskipy # 安装方案二，不能修改默认源
```
...
Commands
    ...
    activate            打印虚拟环境激活脚本
    purge               清理虚拟环境
$ pdm use [python] # 当前 pdm 环境使用哪个 python 解释器
$ pdm venv create [python] [options]
Position
...
$ pdm run python demo.py # 在虚拟环境中执行命令
Options
...
```term
$ pdm config --local pypi.verify_ssl False # 修改项目本地配置
$ pdm config pypi.verify_ssl False # 修改全局配置
```
...
```term
$ pdm sync # 根据 pdm.lock 安装包
$ pdm update # 根据 pyproject.toml 更新 pdm.lock
$ pdm lock # 生成锁定文件
$ pdm lock --check # 检查锁定文件
$ pdm lock --refresh # 刷新锁定文件
```
...
```term
$ uv venv # 根据当前 python 在当前目录创建 .venv 
$ uv venv --python=3.10 # 会下载指定版本 python，然后创建 .venv
$ uv run <command/script> # 有 .venv 则用 .venv 的 python，没有则用当前环境的 
```
...
$ tree . 
    ...
    ├── uv.lock                 版本锁定文件，自动生成
    └── pyproject.toml          项目配置
$ uv add <package> # 添加包，会自动更新 pyproject.toml
Options
...
$ uv remove <package> # 删除包，会自动更新 pyproject.toml
$ uv tree  # 查看包依赖
Resolved 2 packages in 2ms
    uv-test v0.1.0
    └── numpy v2.3.3
$ uv lock # 根据 pyproject.toml 更新 uv.lock 文件
