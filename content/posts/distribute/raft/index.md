...
date: "2025-07-20"
draft: false
description: "Raft共识算法详解，涵盖领导者选举、日志复制及安全性分析，解决脑裂与网络分区问题。"
weight: 16
tags: ["Raft", "共识算法", "分布式一致"]
categories: ["协议设计"]
---
...
![alt](./resources/raft_heartbeat_election.gif)
...
![alt](./resources/raft_partition.gif)
...
![alt](./resources/commit_entry.png)
...
- `S2` 通知 `S1` 同步日志，`S1` 记录了前 `term = 5` 任领导的异常操作
  ...
  ![alt](./resources/raft_log_error_1.png)
...
- `S2` 通知 `S1` 同步日志，`S1` 记录了前 `term = 4` 任领导的异常操作
  ...
  ![alt](./resources/raft_log_error_2.png)
...
- `S2` 通知 `S1` 同步日志，`S1` 未记录到数据
  ...
  ![alt](./resources/raft_log_error_3.png)
