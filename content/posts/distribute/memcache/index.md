...
date: "2025-08-02"
draft: false
description: "Memcache分布式缓存架构，分析单机服务扩展至多节点缓存集群的设计思路。"
weight: 13
tags: ["Memcache", "分布式缓存", "读写分离"]
categories: ["数据存储"]
---
...
![alt](./resources/server_single.png)


2. **负载均衡** ： 将网络响应分布到不同前台服务，前台服务连接同一个数据库


![alt](./resources/server_balance.png)

3. **`RDS`** : 当一个数据库顶不住了，那就通过 `RDS` 将数据库数据「分片」存储到多个数据库节点中

![alt](./resources/server_balance_rds.png)
...
![alt](./resources/server_memcache.png)
...
![alt](./resources/server_facebook.png)
