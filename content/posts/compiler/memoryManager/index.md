...
date: "2025-02-16"
draft: false
description: "讨论内存管理机制，包括缓存原理、垃圾回收可达性分析及标记清除算法。"
weight: 10
tags: ["内存管理", "垃圾回收", "缓存"]
categories: ["运行时"]
---
...
1. 从 `root` 开始，更新所有对象的标记 `mark`
    ...
    ![alt](./resources/mark.png)

2. 遍历堆内存，获取所有的不可达对象

    ![alt](./resources/sweep.png)
...
1. 再将 `new space` 内存区域划分为三个区域
    ...
    ![alt](./resources/stopCopy_1.png)

2. `old space` 空间用完，启动垃圾回收

    ![alt](./resources/stopCopy_2.png)
