...
date: "2025-02-16"
draft: false
description: "介绍代码优化技术，包括局部优化、常量传播及图着色寄存器分配。"
weight: 9
tags: ["代码优化", "常量传播", "寄存器分配"]
categories: ["编译器后端"]
---
...
![alt](./resources/igen_add.png)


中间语言的文法表达

![alt](./resources/intermediateRepresentation.png)
...
![alt](./resources/controlFlowGraph_example.png)
...
2. 初始化所有的状态为 $c(s,x,in) = c(s,x,out) = \bot$
    ...
    ![alt](./resources/constExample_init.png)

4. **前项状态开始运用规则更新变量状态**

    ![alt](./resources/constExample1.png)


### 循环分析


![alt](./resources/constLoop.png)
...
<video src="./resources/constLoop.mp4" controls="controls" width="100%" height="100%"></video>
...
1.  $L(p,x,out) = L(s_1, x, in) \  or \ \dotsm \ or \ L(s_i, x, in)$
    ...
    ![alt](./resources/livenessRule1.png)

2. 若在状态 $s$ 使用了变量 $x$，则 $L(s,x,in) = true$

    ![alt](./resources/livenessRule2.png)

3. 若 $x$ 再次赋值未使用旧的 $x$，则 $L(x := e, x, in) = false$

    ![alt](./resources/livenessRule3.png)

4. 若 $s$ 未使用变量 $x$，则 $L(s,x,in) = L(s,x,out)$

    ![alt](./resources/livenessRule4.png)

**算法处理流程**
1. 设置所有状态的初始值 $L(...) = false$
2. **从后继状态开始更新变量在所有状态下的活跃性**

![alt](./resources/livenessExample.png)
...
<video src="./resources/RIG_color.mp4" controls="controls" width="100%" height="100%"></video>

得到上好色的 RIG 图后，就知道要为临时变量分配哪个寄存器

![alt](./resources/RIG_color.png)

![alt](./resources/registerCode1.png)
