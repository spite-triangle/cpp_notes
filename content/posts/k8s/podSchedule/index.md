...
date: "2025-09-20"
draft: false
description: "CronJob定时任务、污点容忍机制及节点Pod亲和性调度策略。"
weight: 9
tags: ["K8s调度", "亲和性", "CronJob"]
categories: ["部署运维"]
---
...
```term
$ kubectl taint nodes <node> <key>=<value>:<effect> # 污点定义
effect
    NoSchedule                不能容忍该污点的 Pod 将不会部署到该节点，已经部署的不受影响
    NoExecute                 不能容忍该污点的 Pod 将不能在该节点运行，已经部署的会被驱逐
$ kubectl taint nodes <node> <key>:<effect>- # 删除污点
$ kubeclt describe node <node> # 查看污点情况
```
