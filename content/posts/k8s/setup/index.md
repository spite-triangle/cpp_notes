...
date: "2025-09-14"
draft: false
description: "Kubernetes集群搭建方案，涵盖 kubeadm/minikube安装及kubectl命令行用法。"
weight: 3
tags: ["kubeadm", "kubectl", "集群部署"]
categories: ["部署运维"]
---
...
![alt](./resources/docker_kubeadm.png)

### 多节点

![alt](./resources/docker_kind.png)
...
![alt](./resources/minikube_schema.png)
...
```term
$ minikube start # 启动 k8s 集群 ，并自动安装 k8s
Options
    --driver                        容器运行时
...
$ minikube dashboard # k8s web 管理界面
```

### 修改源

```term
$ minikube ssh # 登录 minikube 模拟的 Node 容器
$ sudo vi /etc/docker/daemon.json # 修改镜像地址
...
```term
$ minikube service # 快速实现 k8s 中的 service 功能，实现外部访问集群
```
...
$ minikube ip # 获取集群 IP
$ minikube ssh # 登录 minikube 模拟的 Node 容器
$ minikube node list # 列出所有节点
$ minikube node add # 添加一个节点
$ minikube node delete <name>  # 删除指定节点
$ minikube node start <name>  # 启动节点
$ minikube node stop <name>  # 暂停节点
```
...
$ minikube addons enable <name> # 启动插件
$ minikube addons disable <name> # 停用插件
$ minikube plugin install <name> # 安装
$ minikube plugin uninstall <name> # 卸载
```
...
```term
$ sudo apt install kubelet # 在节点上启动 Pod 和容器的工具
$ sudo apt install kubectl # 管理 k8s 集群的工具
$ sudo apt install kubeadm # 集群的部署工具
```
...
```term
$ kubeadm token list # 在主节点查询 token
...
$ kubeadm join <ip:port> --token <token> --discovery-token-ca-cert-hash <hash> # 加入主节点
```

### CNI 插件

```term
$ kubectl get pod -n kube-system # 系统 pod 没有安装到默认命名空间
...
```term
$ kubeclt apply -f <xxx.yaml> # 安装应用，会自动创建对应 pod
```
