...
date: "2025-09-13"
draft: false
description: "Kubernetes核心概念与术语，包括资源对象层级、控制器及存储机制。"
weight: 2
tags: ["Pod", "控制器", "k8s术语"]
categories: ["基础概念"]
---
...
![alt](./resources/resource_type.png)
...
- **无状态服务部署**
  ...
  - `Deployment` : 对 `RS` 功能进一步增强，**最常用**
    ...
    - 滚动升级/回滚: 当要升级 `RS1` 中 `Pod` 时，`Deployment` 首先会新建一个 `RS2` ，然后在 `RS2` 创建一个新的 `Pod` 并删除 `RS1` 中一个旧的 `Pod`，重复操作，直到 `RS1` 中旧 `Pod` 全部更新完成。滚动回滚同理
      ...
      ![alt](./resources/scroll_update.png)
...
![alt](./resources/service_find.png)
...
![alt](./resources/ingress_service.png)
