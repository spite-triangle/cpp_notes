...
draft: false
slug: kit-redis
description: "Redis内存数据库核心概念与使用：五大数据结构、RDB/AOF持久化方案与主从复制"
weight: 3
tags: ["Redis", "缓存", "NoSQL", "持久化", "主从复制"]
categories: ["后端开发"]
---
...
```term
$ docker pull redis
$ docker run -it -p 6379:6379 --name redis <image id> /bin/bash
$ redis-
redis-benchmark
redis-check-aof
redis-check-rdb
...
![alt](./resources/redis_string.png)
...
![alt](./resources/redis_hash.png)


```term
$ hset key field value # 将 field-value 添加到 key 组下
$ hset key field  # 获取 key.field 的值
$ hexists key field  # 检查 key.field  存在性
$ hdel key field  # 删除 key.field 
$ hincrby key field <n>  #  key.field 的值增加 n
$ hlen key   # 查看 key 下有多少个 field 
$ hkeys key   # 查看 key 下所有的 field 
$ hvals key   # 查看 key 下所有的 value 
$ hgall key   # 查看 key 下所有的 field-value 
```
...
![alt](./resources/redis_session.png)
...
![alt](./resources/redis_list.png)


```term
$ rpush key values # 在 list 左边添加数据
$ lpush key values # 在 list 右边添加数据
$ lrange key i j # 查看 list 中 [i,j] 范围内的数据
- '-1' 表示逆向
- 索引从 0 开始
$ lpop key # 删除左边第一个
$ rpop key # 删除右边第一个
$ llen key # list 长度
$ linsert key before i value # 在 i 对应元素之前插入 value
$ lset key i value # 更改 i 对应元素的值
$ lindex key i # 查看 i 对应元素的值
```
...
![alt](./resources/redis_set.png)


```term
$ sadd key members # 插入值
$ scard key # set 长度
$ smembers key  # 展示所有值
$ srem key members # 删除
$ spop key n # 从集合随机挑出 n 个元素
$ sdiff key1 key2 # 差集 
$ sinter key1 key2 # 交集
$ sunion key1 key2 # 并集
$ s___store dest key1 key2 # 运算结构存储到 dest 中
$ sismember key member # 查看 member 是否在集合 key 中
```
...
```term
$ zadd key score member # 添加 member 元素，其分数为 score
$ zrange key i j # 按照分数升序排序后，展示 [i,j] 索引的元素
$ zrevrange key i j # 按照分数降序排序后，展示 [i,j] 索引的元素
$ zrange key i j withscores # 展示分数值
$ zrank key member # 查看 member 排序后的索引
$ zrevrank key member # 查看 member 排序后的索引
$ zincrby key n member # 给 member 元素的分数加 n
$ zcard key # 查看集合个数
$ zcount key min max # 查看分数在 [min,max] 范围的元素个数
$ zrangebyscore key min max # 查询 [min,max] 分数范围的元素，且排序
```
...
```term
$ keys pattern # 根据 pattern 查询 key
    *   模糊匹配，不要随便乱用，会卡住 Redis
$ exists key # 判断 key 是否存在
$ expire key n # 给存在的 key 设置 n 秒过期
$ persist key # 取消 key 的过期时间
$ select n  # 切换 Redis 到第 n 个数据库， 取值范围 [0,15] 
$ move key n # 将 key 移动到 n 数据库
$ randomkey # 随机生成 key
$ dbsize # 查看数据库中有多少 key
$ echo message # 打印信息
$ info # 查看信息
$ config get * # 查看配置信息
$ flushdb # 删除当前数据库
...
```term
$ SCRIPT LOAD "return 'hello'" # 将脚本缓存
"1b936e3fe509bcbc9cd0664897bbe8fd0cac101b"
$ EVALSHA 1b936e3fe509bcbc9cd0664897bbe8fd0cac101b 0 # 执行缓存中的脚本
$ SCRIPT FLUSH # 清空所有脚本缓存
$ SCRIPT EXISTS 1b936e3fe509bcbc9cd0664897bbe8fd0cac1012 # 检测脚本存在性
```
...
```term
$ save # 会阻塞 Redis，等待 dump 完成
$ bgsave # fork 子进程进行 dump
$ save m n # m 秒内有 n 个 key 发生改变，则使用 bgsave 创建快照
```
...
```term
$ appendfsync always  # 发现写命令就更新日志
$ appendfsync everysec # 每一秒进行一次日志更新
$ appendfsync no    # 一般 30s 执行一次
```
...
$ vim redis.conf # windows 叫 redis.windows.conf
# 设置 redis 密码
requirepass 11111 
    ...
$ redis-cli -a # 密码登陆
...
![alt](./resources/redis_master_slave.png)
...
```term
$ redis-server master.conf # 启动主节点
$ redis-server slave.conf  # 启动从节点
$ redis-cli -h <ip> -p <port> info Replication # 查看服务信息
$ redis-cli -h <ip> -p <port> # 连接不同的 redis 服务
```
