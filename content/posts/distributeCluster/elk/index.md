...
date: "2025-09-23"
draft: false
description: "ELK Stack日志分析系统完整部署：Elasticsearch存储+Logstash处理+Kibana可视化"
weight: 8
tags: ["Elasticsearch", "Logstash", "Kibana", "Filebeat", "日志收集"]
categories: ["基础设施"]
---
...
```term
$ docker run -it --name elastic -p 9200:9200 -m 1g -e "discovery.type=single-node" elasticsearch:8.15.0
Options
...
```term
$ docker exec -it elastic /usr/share/elasticsearch/bin/elasticsearch-reset-password -u elastic # 密码
$ docker exec -it elastic /usr/share/elasticsearch/bin/elasticsearch-create-enrollment-token -s kibana # 生成 token 
```

3. 拷贝挂载内容

```term
$ docker cp elastic:/usr/share/elasticsearch/config /home/triangle/workspace/elk/elastic # 配置
$ docker cp elastic:/usr/share/elasticsearch/data /home/triangle/workspace/elk/elastic # 数据
$ docker cp elastic:/usr/share/elasticsearch/plugins /home/triangle/workspace/elk/elastic # 插件
$ docker cp elastic:/usr/share/elasticsearch/logs /home/triangle/workspace/elk/elastic # 日志
```
...
```term
$ curl --cacert /home/triangle/workspace/elk/elastic/config/certs/http_ca.crt -u elastic:"HlQ-nEWHIIQ7iODcDcTz" https://localhost:9200
Options
    --cacert                证书，配置文件夹下
    -u                      账号:密码，密码第一次运行时会显示
...
```term
$ docker run  -it --name kibana -p 5601:5601 kibana:8.15.0 
   ...
...
2. 填写 `Elasticsearch` 的 `token`
    ...
    ![alt](./resources/Kibana_token.png)

3. 登录 `Elasticsearch` 的账号与密码

    ![alt](./resources/Kibana_login.png)
...
```term
$ docker run -d --name logstash -p 9600:9600 -p 5044:5044 logstash:8.15.0
```

2. 复制配置

```term
$ docker cp logstash:/usr/share/logstash/config /home/triangle/workspace/elk/logstash/
$ docker cp logstash:/usr/share/logstash/pipeline /home/triangle/workspace/elk/logstash/
$ cp -r /home/triangle/workspace/elk/elastic/config/certs/ /home/triangle/workspace/elk/logstash/config/certs # 复制 elastic 的证书
```
...
```
  ...
  ![alt](./resources/elastic_host.png)
...
![alt](./resources/elastic_home.png)



![alt](./resources/elastic_cluster.png)
...
```term
$ vim /home/triangle/workspace/elk/filebeat/filebeat.yml # 创建配置文件
```
...
```term
$ echo "test" >> /home/triangle/workspace/elk/target/server.log
$ echo "test" >> /home/triangle/workspace/elk/target/server.log
$ echo "test 1" >> /home/triangle/workspace/elk/target/server.log
```

![alt](./resources/elastic_log.png)
