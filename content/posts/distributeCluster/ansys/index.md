...
date: "2025-03-30"
draft: false
description: "Ansys HPC高性能计算集群配置与RSM远程求解管理器部署，含ARC单节点和多节点搭建方法"
weight: 6
tags: ["Ansys", "HPC", "RSM", "ARC", "并行计算"]
categories: ["基础设施"]
---
...
![alt](./resources/cluster-diagram.png)
...
![alt](./resources/rsm-interface.png)
...
```term
$ set ARC_ROOT = %AWP_ROOTxxx%\RSM\ARC
```
...
```term
$ arcsubmit [Options] Script_Name [Script_Args] # 提交一个任务到 ARC 中，并返回任务 ID
Options
    -n                  任务使用几核
    -q                  任务发送到的集群队列名
    -a                  任务别名
...
```term
$ arcconfig node modify [execHostName] [options]
Options
    -mn [masterHostName]                    将执行节点关联到头节点，在执行节点上运行
    -c [Cores]                              限制执行节点核数
    -r d:10gb,m:10gb                        限制节点的内存和磁盘使用量
$ arcnodes [nodename] # 查看所有节点状态
```
...
```term
$ arcconfig queue add [queueName] # 创建队列
$ arcconfig queue remove [queueName] # 删除队列
$ arcconfig queue modify [queueName] [options] # 配置队列
Options
...
3. 点击 `Install Electromagnetics RSM` 安装 RSM
   ...
   ![alt](./resources/aedt_rsm.png)

4. 点击 `Install xxx MPI`，**参与集群运算的机器都要安装一样的**

   ![alt](./resources/aedt_mpi.png)

5. 点击 `Register with RSM` 启动 RSM 服务

   ![alt](./resources/aedt_register.png)

6. 启动 MPI 

```term
$ cd "%I_MPI_ROOT%/em64t/bin/"
$ hydra_service -install      # 安装 MPI 服务
$ hydra_service -start        # 启动 MPI 服务
$ mpiexec -register           # 生成 MPI 账号
