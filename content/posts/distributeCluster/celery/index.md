...
date: "2025-08-31"
draft: false
description: "Celery分布式异步任务队列框架使用指南：涵盖worker创建、消息调度与定时任务配置"
weight: 4
tags: ["Celery", "Python", "异步任务", "消息中间件", "任务调度"]
categories: ["后端开发"]
---
...
```term
$ celery -A workers worker  -l info -n name
Options:
...
```term
$ pip install gevent # win10 必须安装这个
$ celery -A workers worker  -l info -P gevent # win10 必须使用 gevent
```
...
```term
$ celery -A celery_workers.celery worker -l info -P eventlet 
Options
...
```term
$ celery  -A celery_workers  worker -l info 
$ celery -A celery_workers beat  # 启动定时器生产者
```
...
![alt](./resources/celery_command.png)


## 命令行

```term
$ celery [command] --help # 帮助
$ celery -A proj status # 展示存活的节点
->  celery@worker2: OK
->  celery@worker1: OK
$ celery -A proj result 4e196aa4-0141-4601-8138-7aa33db0f577 # 查看任务结果
Options
    -t                  指定任务名，例如 '-t tasks.add' 
$ celery -A proj purge # 删除中间件中消息
Options
    -Q LIST                    指定要清空的队名 '-Q q1,q2,q3'
    -X LIST                    不需要清空的队列名
$ celery -A proj inspect [command] [options] # 监控工作节点
commands
    active                  工作节点中正在执行的任务
...
options
    ...
    -d LIST                 指定需要监控的工作节点名
    --timeout               命令执行的超时时间
$ celery -A proj control [command] [options] # 控制工作节点
$ celery -A proj migrate src dest # 数据迁移
```
...
![alt](./resources/celery_event.png)
...
```term
$ pip install curses # 需要安装 curses 库，但只支持 linux
$ celery -A proj events # 简单的事件监听器
Options
    -c                  可指定自定的 Camera
...
```term
$ pip install flower # 安装
$ celery -A proj flower --port=5555  # 启动 flower
```

使用 `http://localhost:5555/` 便能访问 flower 界面

![alt](./resources/flower.png)
...
![alt](./resources/worker_graph_full.webp)
