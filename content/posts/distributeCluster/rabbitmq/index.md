...
date: "2025-07-26"
draft: false
description: "RabbitMQ消息队列协议(AMQP)实现指南：交换机路由模式、队列类型及生产消费者模型"
weight: 2
tags: ["RabbitMQ", "AMQP", "消息队列", "中间件", "Erlang"]
categories: ["后端开发"]
---
...
```term
$ docker pull rabbitmq:4.0.7-management # 下载镜像
```
...
$ docker run -id --name=rabbitmq -p 5671:5671 -p 5672:5672 -p 4369:4369 -p 15671:15671 -p 15672:15672 -p 25672:25672 -e RABBITMQ_DEFAULT_USER=admin -e RABBITMQ_DEFAULT_PASS=admin 87d72938da08
RABBITMQ_DEFAULT_USER # 默认账号名
ABBITMQ_DEFAULT_PASS # 默认密码
$ docker ps -a # 查看运行的 rabbitmq 容器
docker ps -a
CONTAINER ID   IMAGE          COMMAND                  CREATED          STATUS          PORTS
                    NAMES
916fa3408d42   87d72938da08   "docker-entrypoint.s…"   28 minutes ago   Up 28 minutes   ...   jovial_brown
$ docker exec -it 916fa3408d42 /bin/bash # 进入容器
```
...
$ service rabbitmq-server stop # 暂停服务
```

```term
$ rabbitmq-server -detached # 后台启动服务，docker 版本使用
```
...
```term
$ rabbitmqctl start_app # 直接启动 rabbitmq 程序
...
```term
$ rabbitmqctl add_user admin # 创建用户
$ rabbitmqctl set_permissions -p / admin "." "." ".*" # 设置权限
$ rabbitmqctl set_user_tags admin administrator # 设置管理员
```

## rabbitmq-plugins

```term
$ rabbitmq-plugins list # 查看插件启用状况
...
![alt](./resources/synchronizeModel.png)
...
![alt](./resources/mqModel.png)

## RabbitMQ 框架


![alt](./resources/rabbitmqModel.png)
...
![alt](./resources/rabbitmqProcess.png)
...
![alt](./resources/webRabbitQueue.png)
...
- 交换机
    ...
    ![alt](./resources/webExchange.png)

- 通道

    ![alt](./resources/webChannel.png)

- 用户

    ![alt](./resources/webUser.png)

- 虚拟主机

    ![alt](./resources/webVirtualHost.png)

- 消息与绑定

    ![alt](./resources/webMessageBind.png)

- 策略

    ![alt](./resources/webAdminPolicy.png)
...
![alt](./resources/rabbitmqDirect.png)
...
![alt](./resources/webRabbitmqDirect.png)
...
![alt](./resources/webRabbitmqFount.png)
...
![alt](./resources/rabbitmqTopic.png)
...
![alt](./resources/rabbitmqHeaders.png)
...
![alt](./resources/webChannelClassic.png)
...
![alt](./resources/webQueueQuorum.png)
...
![alt](./resources/webQueueStream.png)
