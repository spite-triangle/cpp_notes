...
date: "2024-05-12"
draft: false
description: "WebAssembly通过Emscripten将C/C++源码编译为浏览器可直接运行的wasm二进制模块。"
weight: 7
tags: ["WebAssembly", "WASM", "Emscripten", "emcc", "javascript交互"]
categories: ["Web开发", "C++应用"]
---
...
```term
$ git clone https://github.com/emscripten-core/emsdk.git # 下载项目
$ cd emsdk
$ ./emsdk install latest # 下载最新版本
1. emsdk 文件
...
![hello](./resources/wasm_hello.jpg)
...
```term
$ emcc hello.c -o sum.js -s EXPORTED_FUNCTIONS=_main,_sum -s WASM=1
...
```term
$ emcc -g -fdebug-compilation-dir='.' hello.cpp -o hello.html
Options:
    -g                          生成调试信息
    -fdebug-compilation-dir     源码相对于 hello.html 的路径，通过 http 协议加载。若不指定则通过 file:// 协议获取本地源文件
```

![DWARF](./resources/dwarf.jpg)

## Source Map

```term
$ emcc -gsource-map hello.c -o hello.html
Options:
...
![source map](./resources/sourcemap.jpg)
