...
date: "2025-10-29"
draft: false
description: "Kafka简介与安装部署，涵盖点对点与发布订阅通信模式。"
weight: 1
tags: ["Kafka", "消息队列", "入门教程"]
categories: ["基础概念"]
---
...
![alt](./resources/model_single.png)
...
$ cd /opt/kafka/bin # 进入 kafka 命令脚本位置
$ ls -l
total 172
...
```term
$ /etc/kafka/docker/run # 快捷启动 kafka
$ ./kafka-server-start.sh /etc/kafka/docker/server.properties # 启动服务
...
```term
$ ./kafka-topics.sh  --bootstrap-server localhost:9092 --create --topic test # 创建 topic
Options
    --bootstrap-server          kafka 服务 host
    --create                    创建
    --topic                     topic 名 (_ 与 . 在命名中不用同时使用)
$ ./kafka-topics.sh  --bootstrap-server localhost:9092 --list # 罗列所有 topic
$ ./kafka-topics.sh  --bootstrap-server localhost:9092 --topic test --describe # 查看 topic 详细信息
$ ./kafka-topics.sh  --bootstrap-server localhost:9092 --topic test --alter --<key> <value> # 修改配置
$ ./kafka-topics.sh  --bootstrap-server localhost:9092 --topic test --alter --delete # 在 window 中，可能会导致 kafka 异常关闭
```

## consumer / producer

```term
$ /opt/kafka/bin$ ./kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic test # 控制台消费者
```

```term
$ /opt/kafka/bin$ ./kafka-console-producer.sh --bootstrap-server localhost:9092 --topic test # 控制台消费者
```
...
$ docker run -d -p 9000:9000 --name kafdrop -e KAFKA_BROKERCONNECT=<kafka host:ip> obsidiandynamics/kafdrop
```
