...
date: "2025-10-29"
draft: false
description: "Kafka生产者消息发送全流程，涵盖序列化、分区策略与事务机制。"
weight: 5
tags: ["生产者", "消息可靠性", "ACK确认"]
categories: ["核心概念"]
---
...
![alt](./resources/kafka_produce.png)
...
- **数据重复** : `data` 发送到 `broker` 且 `broker` 保存成功，但是响应 `producer` 时异常，这时 `producer` 就会重新发送一次 `data` 请求，导致 `broker` 日志中消息重复
    ...
    ![alt](./resources/retry_repeat.png)
