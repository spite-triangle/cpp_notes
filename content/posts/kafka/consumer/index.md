...
date: "2026-02-07"
draft: false
description: "Kafka消费者偏移量管理与消费组，涵盖分区分配策略RoundRobin等。"
weight: 7
tags: ["消费者", "消费组", "负载均衡"]
categories: ["核心概念"]
---
...
![alt](./resources/consumer_fetch.png)
...
![alt](./resources/consumer_group.png)
...
1. 每个消费者组中的消费者都会含有一个自动生产的`UUID`作为`memberid`
    ...
    ![alt](./resources/roundAssignor_1.png)

2. 将每个消费者按照 `memberid` 进行排序，所有 `member` 消费的主题分区根据主题名称进行排序

    ![alt](./resources/roundAssignor_2.png)

3. 将主题分区轮询分配给对应的订阅用户，注意未订阅当前轮询主题的消费者会跳过

    ![alt](./resources/roundAssignor_3.png)

    ![alt](./resources/roundAssignor_4.png)
...
![alt](./resources/rangeAssignor.png)
...
![alt](./resources/stickyAssignor_1.png)
