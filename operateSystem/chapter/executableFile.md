...
对于 `GCC` 而言，比较符合认知。静态库的符号在「链接」时就会完成重定向，且进行静态库与动态库的源码符号相同时，并不会产生重定义错误；当可执行文件「加载」时，遇到相同的源码符号也不会再做修改。**根本原因应该是相同的源码符号，在动态库中会添加 `@plt` 进行区分（猜的，未求证过）。**

- **MSVC**

`MSVC` 则要更为苛刻一些，在「链接」时，静态库与动态库的源码符号相同时，会产生重定义错误
