...
2. 从`MCP`仓库找到找到配置，**有些`server`可能有问题，多换几个**
    ...
    ![alt](../../image/theory/mcp_vscode.png)

3. 配置成功后，大模型便能自动调用对应的 `MCP` 工具

  ![alt](../../image/theory/mcp_hello.png)
...
1. 安装 `cline` 插件
  ...
  ![alt](../../image/theory/mcp_cline.png)

2. 登录 `cline` 或者配置第三方的 `LLM` 使用

3. 配置 `MCP` 或者从市场安装配置

  ![alt](../../image/theory/mcp_cline_config.png)
...
![alt](../../image/theory/mcp_workerflow.png)
